...
let glitchLayers = [];
let noiseOffset = 0;
let deterioration = 0.0;        // 0 = pristine, 1 = total decay
let midiEvents = [];
let scanlines = [];
let deadPixels = [];
let chromaShift = 0;
...
const LAYERS = 8;
const MAX_DETERIORATION = 100.0;
const DETERIORATION_SPEED = 0.0;
...
window.onMidiEvent = function(event) {
  ...
  let glitchBoost = map(event.velocity, 0, 127, 0, 0.05);
...
function draw() {
  ...
  deterioration = min(deterioration + DETERIORATION_SPEED, MAX_DETERIORATION);
